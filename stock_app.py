...
if df is None or len(df) < 5:
    ...
    # --- 7. OVERALL PERIOD SENTIMENT BAR ---
    st.markdown("---")
    st.subheader("🌐 Aggregate Market Sentiment (All Timeframes)")
    
    master_total = total_buy_all + total_sell_all
    master_buy_pct = (total_buy_all / master_total * 100) if master_total > 0 else 50
    master_sell_pct = 100 - master_buy_pct

    st.markdown(f"""
        <div style="width: 100%; background-color: #444; border-radius: 10px; display: flex; height: 40px; overflow: hidden; border: 1px solid #fff;">
            <div style="width: {master_buy_pct}%; background-color: #26a69a; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; font-size: 16px;">
                BUY {master_buy_pct:.1f}%
            </div>
            <div style="width: {master_sell_pct}%; background-color: #ef5350; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; font-size: 16px;">
                SELL {master_sell_pct:.1f}%
            </div>
        </div>
    """, unsafe_allow_html=True)

    # --- 8. BOTTOM PREDICTION PANEL ---
    st.subheader("🎯 Future Price Target")
    last_close = float(df['Close'].iloc[-1])
    
    if prediction_mode == "Next Day (Tomorrow)":
        target_val = predict_tomorrow(symbol)
        display_label = "Predicted Price for Tomorrow"
    else:
        ...
        display_label = "Estimated Scalp Target (Next Candle)"

    final_target = np.clip(target_val, last_close * 0.95, last_close * 1.05) if target_val else last_close
    change_pct = ((final_target / last_close) - 1) * 100
    bg_color = "#1e3d2c" if change_pct > 0 else "#3d1e1e"

    st.markdown(f"""
        <div style="background-color: {bg_color}; padding: 25px; border-radius: 15px; text-align: center; border: 2px solid white;">
            <h2 style="margin: 0; color: white;">{display_label}</h2>
            <h1 style="font-size: 60px; margin: 10px 0; color: #00ffcc;">₹{final_target:.2f}</h1>
            <h3 style="margin: 0; color: {'#00ff00' if change_pct > 0 else '#ff4d4d'};">
