...
# Using .copy() to prevent Streamlit cache mutation errors
df = pd.concat([df, bb], axis=1).copy()

# 4. Signal Logic (Smart Search Version)
def get_signal(row):
    # This finds the Bollinger Band columns even if the library names them differently
...
# Bollinger Bands (Using 'filter' to find the right columns automatically)
