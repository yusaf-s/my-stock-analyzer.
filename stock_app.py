...
st.set_page_config(layout="wide", page_title="India Alpha: Silverline Edition")
...
# 1. Inputs - Defaulted to Silverline BSE
symbol = st.sidebar.text_input("Ticker", "500389.BO") # Silverline BSE ID
...
if df is None or len(df) < 15:
    st.error("❌ Waiting for Silverline Data... Try changing the Horizon.")
else:
    # 3. Calculations
    df['RSI'] = ta.rsi(df['Close'], length=14)
    bb = ta.bbands(df['Close'], length=20, std=1.5) # Tighter bands for penny stocks
    ...
    fig = make_subplots(rows=3, cols=1, shared_xaxes=True, 
                        ...
                        subplot_titles=('Price & Silverline Signals', 'Volume vs Avg', 'RSI Momentum'))
    ...
    fig.add_trace(go.Scatter(x=buy_pts.index, y=buy_pts['Low']*0.98, mode='markers+text', 
                             ...
                             marker=dict(symbol='triangle-up', color='lime', size=12), name='BUY'), row=1, col=1)
    ...
    # 5. Silverline Confidence Verdict
    st.subheader("Silverline Analysis")
    last_row = df.iloc[-1]
    ...
    if last_row['Buy_Signal']:
        if curr_vol > (avg_vol * 0.8): conf = "🚀 HIGH CONFIDENCE BUYING"
        else: conf = "⚠️ FAKING / WEAK MOVE"
        st.success(f"Signal: BUY | RSI: {rsi_val:.2f} | {conf}")
        
    elif last_row['Sell_Signal']:
        if curr_vol > (avg_vol * 0.8): conf = "💥 HIGH CONFIDENCE SELLING"
        else: conf = "⚠️ FAKING / WEAK MOVE"
        st.error(f"Signal: SELL | RSI: {rsi_val:.2f} | {conf}")
        
    else:
        # Check if the stock is flat/circuit
        st.info(f"Signal: NEUTRAL | RSI: {rsi_val:.2f} | Strength: {'Stable' if curr_vol < avg_vol else 'Active'}")
